...
Vytal does not send, resend, or track invitation email delivery. The raw secret is returned only at creation and cannot be recovered after the page is refreshed because only its hash is stored. Create a new invitation if the secret was not copied. Invitation history shows status and metadata without revealing the secret; an active invitation may be revoked.
