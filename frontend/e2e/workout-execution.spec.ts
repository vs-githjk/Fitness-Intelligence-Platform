...
test('trainee safety flow pauses pain, force-ends critical symptoms, and supports coach review', async ({ page }) => {
  const auth = await credentialSession(page, 'trainee@fitness.example.com')
  const api = await authenticatedApi(auth)
  await page.setViewportSize({ width: 390, height: 844 })
  let data = await workspace(api)
  const painWorkout = data.scheduled_workouts.find(item => item.status === 'scheduled')
  expect(painWorkout).toBeTruthy()
  await page.goto(`/trainee/workouts/${painWorkout!.id}`)
  await page.getByRole('button', { name: 'Start workout' }).click()
  await page.getByRole('button', { name: 'Report a safety concern' }).click()
  await page.getByLabel('What happened?').selectOption('pain')
  await page.getByLabel(/What should your coach know/).fill('Synthetic Playwright pain report.')
  await page.getByRole('button', { name: 'Submit safety report' }).click()
  await expect(page.getByText('Exercise paused for safety')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Next' })).toHaveCount(0)
  await expect(page.getByText('Synthetic Playwright pain report.')).toBeVisible()
  await page.getByRole('button', { name: 'Skip exercise' }).click()
  await page.getByRole('button', { name: 'End workout incomplete' }).click()
  await expect(page.getByText('Workout ended incomplete')).toBeVisible()

  data = await workspace(api)
  const criticalWorkout = data.scheduled_workouts.find(item => item.status === 'scheduled')
  expect(criticalWorkout).toBeTruthy()
  await page.goto(`/trainee/workouts/${criticalWorkout!.id}`)
  await page.getByRole('button', { name: 'Start workout' }).click()
  await page.getByRole('button', { name: 'Report a safety concern' }).click()
  await page.getByLabel('What happened?').selectOption('chest_discomfort')
  await page.getByLabel('Severity').selectOption('severe')
  await page.getByLabel(/What should your coach know/).fill('Synthetic Playwright critical report.')
  await expect(page.getByText('Stop exercising now')).toBeVisible()
  await page.getByRole('button', { name: 'Submit and end workout' }).click()
  await expect(page.getByText('Workout stopped for safety')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Save completed set' })).toHaveCount(0)
  ...
  await page.goto('/coach/safety-reports?phase7a=1')
  await page.getByText('Synthetic Playwright critical report.').click()
  await page.getByLabel(/Internal coach note/).fill('Synthetic coach follow-up.')
  ...
  await expect(page.getByText('Synthetic coach follow-up.')).toBeVisible()
})
...
test('demo execution is read-only, role-protected, and mobile-safe', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Report a safety concern' })).toBeDisabled()
  ...
  const mutationTarget = data.scheduled_workouts[0]
  expect(mutationTarget).toBeTruthy()
  const denied = await api.post(`${apiUrl}/trainee/workouts/${mutationTarget.id}/start`)
