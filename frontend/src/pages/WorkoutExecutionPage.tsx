...
function SessionExperience({ session, onChange, onReload, demo }: { session: WorkoutSession; onChange: (value: WorkoutSession) => void; onReload: () => Promise<unknown>; demo: boolean }) {
  ...
  const paused = exercise?.status === 'paused_for_safety'
  ...
  async function reported(report: WorkoutSafetyReport) {
    cache.setQueryData<WorkoutSafetyReport[]>([...scope, 'workout-safety-reports', session.id], current => [report, ...(current ?? [])])
    await onReload()
    setSafetyOpen(false)
  }
...
function SafetyReportDialog({ open, session, exercise, draft, demo, onDraft, onClose, onReported }: { open: boolean; session: WorkoutSession; exercise?: WorkoutSessionExercise; draft: SafetyDraft; demo: boolean; onDraft: (value: SafetyDraft) => void; onClose: () => void; onReported: (report: WorkoutSafetyReport) => Promise<void> }) {
  ...
  const critical = criticalCategories.includes(draft.category)
  const submit = useMutation({
    mutationFn: () => api<WorkoutSafetyReport>(`/trainee/workout-sessions/${session.id}/safety-reports`, { method: 'POST', body: JSON.stringify({ workout_session_exercise_id: exercise?.id ?? null, category: draft.category, severity: draft.severity, note: draft.note || null, activity_stopped: critical || draft.activityStopped }) }),
    onSuccess: report => { setMessage('Safety report submitted.'); void onReported(report) },
    onError: caught => setMessage(caught instanceof Error ? caught.message : 'The safety report could not be submitted.'),
  })
  return <Modal open={open} onClose={onClose} title="Report a safety concern" description="This records what happened for your coach. Reports are not monitored continuously and are not a medical diagnosis."><StatusNotice tone={critical ? 'risk' : 'attention'} title={critical ? 'Stop exercising now' : 'Put safety first'}>{critical ? 'Submitting this report will end the workout. If symptoms are severe, worsening, or continue, seek urgent professional medical assistance.' : 'Stop or change the activity if needed.'}</StatusNotice><div className="mt-4 space-y-4"><Field label="What happened?">{({ id }) => <SelectInput id={id} value={draft.category} disabled={demo || submit.isPending} onChange={event => onDraft({ ...draft, category: event.target.value as SafetyCategory })}><option value="pain">Pain</option><option value="unusual_discomfort">Unusual discomfort</option><option value="chest_discomfort">Chest discomfort</option><option value="breathing_difficulty">Breathing difficulty</option><option value="dizziness_or_faintness">Dizziness or faintness</option><option value="loss_of_balance">Loss of balance</option><option value="equipment_or_environment">Equipment or environment</option><option value="other">Other</option></SelectInput>}</Field><Field label="Severity">{({ id }) => <SelectInput id={id} value={draft.severity} disabled={demo || submit.isPending} onChange={event => onDraft({ ...draft, severity: event.target.value as SafetySeverity })}><option value="mild">Mild</option><option value="moderate">Moderate</option><option value="severe">Severe</option></SelectInput>}</Field><Field label="What should your coach know?" optional>{({ id }) => <TextArea id={id} maxLength={500} value={draft.note} disabled={demo || submit.isPending} onChange={event => onDraft({ ...draft, note: event.target.value })} />}</Field><label className="flex min-h-11 items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={critical || draft.activityStopped} disabled={demo || critical || submit.isPending} onChange={event => onDraft({ ...draft, activityStopped: event.target.checked })} />I stopped the activity</label>{message && <p role="status" className="text-sm font-semibold text-secondary">{message}</p>}<div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button variant="secondary" onClick={onClose}>Keep draft and close</Button><Button variant="danger" disabled={demo} loading={submit.isPending} onClick={() => submit.mutate()}><ShieldAlert aria-hidden="true" className="size-4" />{critical ? 'Submit and end workout' : 'Submit safety report'}</Button></div></div></Modal>
}

function SafetyHistory({ reports }: { reports: WorkoutSafetyReport[] }) {
...
function ExercisePanel({ session, exercise, demo, onChange, onConflict, onLogged }: { session: WorkoutSession; exercise: WorkoutSessionExercise; demo: boolean; onChange: (value: WorkoutSession) => void; onConflict: () => void; onLogged: (restSeconds: number | null, nextLabel: string) => void }) {
  ...
  const [message, setMessage] = useState('')
  const immutable = ['skipped', 'safety_stopped'].includes(exercise.status)
...
function FinishPanel({ session, demo, onChange, onConflict }: { session: WorkoutSession; demo: boolean; onChange: (value: WorkoutSession) => void; onConflict: () => void }) {
  ...
  const [endReason, setEndReason] = useState('time_constraint')
  const [message, setMessage] = useState('')
