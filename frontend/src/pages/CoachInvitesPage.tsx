...
export function CoachInvitesPage() {
  ...
  return <AppShell><div className="space-y-8"><PageHeader eyebrow="Coach workspace" title="Trainee invitations" description="Create private, single-use invitations that assign each trainee directly to you." />
    <Card><h2 className="text-xl font-semibold">Create an invitation</h2><p className="mt-1 text-sm leading-6 text-secondary">Email delivery is not included. Copy the code or link and share it through a trusted channel.</p><form onSubmit={create} className="mt-5 grid gap-5 sm:grid-cols-[1fr_12rem_auto]"><Field label="Trainee email" optional help="If supplied, only this email can redeem the invitation.">{({ id, describedBy, invalid }) => <TextInput id={id} name="intended_email" type="email" autoComplete="off" aria-describedby={describedBy} aria-invalid={invalid} />}</Field><Field label="Expires after">{({ id, describedBy, invalid }) => <SelectInput id={id} name="expires_in_days" defaultValue="7" aria-describedby={describedBy} aria-invalid={invalid}><option value="1">1 day</option><option value="3">3 days</option><option value="7">7 days</option><option value="14">14 days</option><option value="30">30 days</option></SelectInput>}</Field><Button type="submit" loading={busy} className="self-end"><MailPlus aria-hidden="true" className="size-4" />Create invite</Button></form></Card>
    {created && <StatusNotice tone="positive" title="Invitation created—copy it now"><p>The secret is shown only in this response. It cannot be recovered after this page is refreshed.</p><div className="mt-3 grid gap-3"><div className="flex gap-2"><code className="min-w-0 flex-1 overflow-x-auto rounded-lg bg-surface px-3 py-2 text-xs text-foreground">{created.token}</code><Button variant="secondary" onClick={() => copy(created.token, 'Code copied')}><Copy aria-hidden="true" className="size-4" />Code</Button></div><div className="flex gap-2"><code className="min-w-0 flex-1 overflow-x-auto rounded-lg bg-surface px-3 py-2 text-xs text-foreground">{inviteLink(created.token)}</code><Button variant="secondary" onClick={() => copy(inviteLink(created.token), 'Link copied')}><Link2 aria-hidden="true" className="size-4" />Link</Button></div>{copied && <p aria-live="polite" className="text-xs font-semibold">{copied}</p>}</div></StatusNotice>}
    {error && <StatusNotice tone="risk" title="Invitation action unsuccessful">{error}</StatusNotice>}
    <section aria-labelledby="invite-history"><h2 id="invite-history" className="text-xl font-semibold">Invitation history</h2><p className="mt-1 text-sm text-secondary">Raw invitation codes are never shown here.</p>{loading ? <p className="mt-5 text-sm text-secondary">Loading invitations…</p> : invites.length === 0 ? <div className="mt-5"><EmptyState icon={UserPlus} title="No invitations yet" description="Create the first invitation to connect a trainee to your workspace." /></div> : <div className="mt-5 grid gap-3">{invites.map(invite => <Card key={invite.id} as="article" className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate font-semibold">{invite.intended_email ?? 'Any invited email'}</p><Badge tone={statusTone(invite.status)}>{invite.status}</Badge></div><p className="mt-1 text-xs text-muted">Created {new Date(invite.created_at).toLocaleString()} · Expires {new Date(invite.expires_at).toLocaleString()}</p></div>{invite.status === 'active' && <Button variant="danger" disabled={busy} onClick={() => revoke(invite)}>Revoke</Button>}</Card>)}</div>}</section>
  </div></AppShell>
}
