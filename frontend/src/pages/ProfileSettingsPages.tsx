...
import { Badge, Button, Card, Field, LoadingState, PageHeader, SegmentedControl, SelectInput, StatusNotice, TextArea, TextInput } from '../components/ui'
...
export function ProfilePage() {
  ...
  return <AppShell>
    <div className="mx-auto max-w-2xl space-y-6">
      <PageHeader eyebrow="Account" title="Your profile" description={isCoach ? 'Your professional profile, shown to the trainees you coach. Self-declared and never verified.' : 'Shared identity used across Vytal. Self-declared and never verified.'} />
...
function PasswordCard({ demo }: { demo: boolean }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api<void>('/me/password', { method: 'PUT', body: JSON.stringify({ current_password: current, new_password: next }) }),
    onSuccess: () => { setError(''); setMessage('Your password was changed. Use it the next time you sign in.'); setCurrent(''); setNext(''); setConfirm('') },
  ...
  return <form onSubmit={submit} noValidate><Card className="space-y-5"><div><h2 className="text-lg font-semibold">Password</h2><p className="mt-1 text-sm text-secondary">Change the password you use to sign in. You will need your current password.</p></div>{message && <StatusNotice tone="positive" title="Password changed">{message}</StatusNotice>}{error && <StatusNotice tone="risk" title="We could not change your password">{error}</StatusNotice>}<Field id="current_password" label="Current password">{({ id, describedBy, invalid }) => <TextInput id={id} type="password" autoComplete="current-password" value={current} onChange={event => setCurrent(event.target.value)} disabled={demo} aria-describedby={describedBy} invalid={invalid} />}</Field><Field id="new_password" label="New password" help="Use at least 10 characters.">{({ id, describedBy, invalid }) => <TextInput id={id} type="password" autoComplete="new-password" value={next} onChange={event => setNext(event.target.value)} disabled={demo} aria-describedby={describedBy} invalid={invalid} />}</Field><Field id="confirm_password" label="Confirm new password">{({ id, describedBy, invalid }) => <TextInput id={id} type="password" autoComplete="new-password" value={confirm} onChange={event => setConfirm(event.target.value)} disabled={demo} aria-describedby={describedBy} invalid={invalid} />}</Field><div className="flex justify-end"><Button type="submit" loading={mutation.isPending} disabled={demo} className="sm:min-w-44">Change password</Button></div></Card></form>
}

// Iron Editorial theme control (C2.1). Real Light / Dark / System preference, wired to
// the unified ThemeProvider (which persists to /me/preferences). The segmented control
// shows the current effective appearance; picking one makes it an explicit choice.
...
function ThemeCard() {
  ...
  const current: string = preference ?? resolved
  return (
    <Card className="space-y-3">
      <div><h2 className="text-lg font-semibold">Theme</h2><p className="mt-1 text-sm text-secondary">Choose how Vytal looks. This changes presentation only and is saved to your account.</p></div>
      <SegmentedControl label="Theme" value={current} options={THEME_OPTIONS} onChange={(value) => setPreference(value as ThemePreference)} />
      <p className="text-xs text-muted">{preference === 'system' ? 'Following your device appearance.' : `Applied on every device where you sign in.`}</p>
    </Card>
  )
}
