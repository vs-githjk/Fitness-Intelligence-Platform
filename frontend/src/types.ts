...
// Trainee exercise-knowledge read view (C2.2, §30) — published-only, read-only.
export interface TraineeExerciseMedia { id: string; purpose: string; content_type: string; byte_size: number; original_filename: string | null; content_url: string }
export interface TraineeExerciseKnowledge { id: string; exercise_id: string; name: string; description: string | null; tracking_mode: ExerciseTrackingMode; category: string; movement_pattern: string; equipment: string[]; primary_muscle_groups: string[]; secondary_muscle_groups: string[]; unilateral: boolean; difficulty: ExerciseDifficulty | null; instructions: string; safety_cues: string[]; coaching_cues: string[]; common_mistakes: string[]; primary_image: TraineeExerciseMedia | null; secondary_image: TraineeExerciseMedia | null; demonstration_video: TraineeExerciseMedia | null }
...
// Workout import (CSV preview → resolve → create draft)
export interface ImportMatchExercise { exercise_id: string; exercise_version_id: string; name: string; tracking_mode: ExerciseTrackingMode; movement_pattern: string }
export interface ImportRowRaw { reps_min: number | null; reps_max: number | null; load: string | null; load_unit: string | null; duration_seconds: number | null; distance: string | null; distance_unit: string | null; rest_seconds: number | null; notes: string | null }
export interface ImportPreviewRow { line: number; exercise_name: string; status: 'matched' | 'needs_review' | 'not_found'; sets: number; matched: ImportMatchExercise | null; candidates: ImportMatchExercise[]; prescription: Record<string, unknown> | null; raw: ImportRowRaw; error: string | null }
export interface ImportPreview { template_name: string; file_errors: string[]; summary: { total: number; matched: number; needs_review: number; not_found: number }; rows: ImportPreviewRow[] }
