...
import { ExerciseDetail, ExerciseDifficulty, ExerciseDraftData, ExerciseTrackingMode } from '../../types'
...
const empty: ExerciseDraftData = { name: '', description: null, instructions: '', tracking_mode: 'repetitions_and_load', category: '', movement_pattern: '', equipment: [], primary_muscle_groups: [], secondary_muscle_groups: [], unilateral: false, safety_cues: [], difficulty: null, coaching_cues: [], common_mistakes: [], image_url: null, thumbnail_url: null }
...
export function ExerciseEditor() {
  ...
  // Refresh the exercise cache after a media change without disturbing unsaved
  // knowledge edits (the draft id is unchanged, so the form is not reloaded).
  const applyDetail = useCallback((next: ExerciseDetail) => { cache.setQueryData([...scope, 'programming-exercise', next.id], next); void cache.invalidateQueries({ queryKey: [...scope, 'programming-exercises'] }) }, [cache, scope])
  ...
  return <ProgrammingShell title={isNew ? 'Create private exercise' : visibleVersion?.name ?? 'Exercise'} description={detail?.scope === 'system' ? 'System exercises are published references and cannot be changed.' : 'Build a private exercise draft, then publish an immutable version.'} action={<Button variant="secondary" onClick={leave}><ArrowLeft aria-hidden="true" className="size-4" />Back to library</Button>}>
    ...
    <form onSubmit={save} className="space-y-6"><Card><div className="grid gap-5 lg:grid-cols-2"><Field label="Name" error={fieldErrors.name} id="exercise-name">{props => <TextInput {...props} value={form.name} disabled={immutable} onChange={event => change('name', event.target.value)} />}</Field><Field label="Tracking mode" help={modes.find(item => item.value === form.tracking_mode)?.help} error={fieldErrors.tracking_mode} id="exercise-tracking_mode">{props => <SelectInput {...props} value={form.tracking_mode} disabled={immutable} onChange={event => change('tracking_mode', event.target.value as ExerciseTrackingMode)}>{modes.map(mode => <option key={mode.value} value={mode.value}>{mode.label}</option>)}</SelectInput>}</Field><Field label="Description" optional error={fieldErrors.description} id="exercise-description">{props => <TextArea {...props} value={form.description ?? ''} disabled={immutable} onChange={event => change('description', nullable(event.target.value))} />}</Field><Field label="Instructions" help="Describe setup and execution clearly for future trainee-facing use." error={fieldErrors.instructions} id="exercise-instructions">{props => <TextArea {...props} value={form.instructions} disabled={immutable} onChange={event => change('instructions', event.target.value)} />}</Field><Field label="Category" error={fieldErrors.category} id="exercise-category">{props => <TextInput {...props} value={form.category} disabled={immutable} onChange={event => change('category', event.target.value)} />}</Field><Field label="Movement pattern" error={fieldErrors.movement_pattern} id="exercise-movement_pattern">{props => <TextInput {...props} value={form.movement_pattern} disabled={immutable} onChange={event => change('movement_pattern', event.target.value)} />}</Field><ListField label="Equipment" help="Comma-separated; leave blank for none." value={form.equipment} disabled={immutable} onChange={value => change('equipment', value)} /><ListField label="Primary muscle groups" help="At least one is required." value={form.primary_muscle_groups} disabled={immutable} error={fieldErrors.primary_muscle_groups} onChange={value => change('primary_muscle_groups', value)} /><ListField label="Secondary muscle groups" value={form.secondary_muscle_groups} disabled={immutable} onChange={value => change('secondary_muscle_groups', value)} /><ListField label="Safety cues" help="Separate concise cues with commas." value={form.safety_cues} disabled={immutable} onChange={value => change('safety_cues', value)} /><Field label="Difficulty" optional help="Instructional difficulty; not a medical rating." error={fieldErrors.difficulty} id="exercise-difficulty">{props => <SelectInput {...props} value={form.difficulty ?? ''} disabled={immutable} onChange={event => change('difficulty', (event.target.value || null) as ExerciseDifficulty | null)}><option value="">Not specified</option>{difficulties.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</SelectInput>}</Field><ListField label="Coaching cues" help="Short instructional cues, comma-separated." value={form.coaching_cues} disabled={immutable} onChange={value => change('coaching_cues', value)} /><ListField label="Common mistakes" help="Frequent errors to watch for, comma-separated." value={form.common_mistakes} disabled={immutable} onChange={value => change('common_mistakes', value)} /><Field label="Image URL" optional help="Legacy external HTTPS image. Prefer uploading media below." error={fieldErrors.image_url} id="exercise-image_url">{props => <TextInput {...props} type="url" value={form.image_url ?? ''} disabled={immutable} onChange={event => change('image_url', nullable(event.target.value))} />}</Field><Field label="Thumbnail URL" optional help="Public HTTPS images only." error={fieldErrors.thumbnail_url} id="exercise-thumbnail_url">{props => <TextInput {...props} type="url" value={form.thumbnail_url ?? ''} disabled={immutable} onChange={event => change('thumbnail_url', nullable(event.target.value))} />}</Field><label className="flex min-h-11 items-center gap-3 rounded-xl border p-3 text-sm font-semibold"><input type="checkbox" checked={form.unilateral} disabled={immutable} onChange={event => change('unilateral', event.target.checked)} className="size-5" />Unilateral exercise</label></div></Card>
    ...
    {!isNew && exerciseId && detail && editableVersion && detail.scope === 'coach_private' && <ExerciseMediaManager exerciseId={exerciseId} version={editableVersion} disabled={Boolean(user?.is_demo)} onChanged={applyDetail} />}
    {visibleVersion && <ExercisePreview version={visibleVersion} />}
...
function copyDraft(value: ExerciseDraftData): ExerciseDraftData { return { ...value, equipment: [...value.equipment], primary_muscle_groups: [...value.primary_muscle_groups], secondary_muscle_groups: [...value.secondary_muscle_groups], safety_cues: [...value.safety_cues], coaching_cues: [...value.coaching_cues], common_mistakes: [...value.common_mistakes] } }
